...
import com.peizhen.cache.ConcurrentMapWithValuedEviction;
...
/**
 * A {@link ConcurrentMapWithValuedEviction} implementation which decorates an
 * existing {@link java.util.concurrent.ConcurrentMap} implementation. This
 * class uses an instance of {@link EvictionValueScheduler} for automatically
 ...
 */
public class ConcurrentMapWithValuedEvictionDecorator<K, V> extends AbstractMap<K, V> implements ConcurrentMapWithValuedEviction<K, V> {

    public final ConcurrentNavigableMap<K, EvictibleValueEntry<K, V>> delegate;

    private final EvictionValueScheduler<K, V> scheduler;
     ...
     */
    public ConcurrentMapWithValuedEvictionDecorator(ConcurrentNavigableMap<K, EvictibleValueEntry<K, V>> delegate, EvictionValueScheduler<K, V> scheduler) {
        super();
     ...
     * been evicted, but since it is a {@link ConcurrentNavigableMap}, it is ok to return
    ...
    public long volumn() {
        ...
        while (it.hasNext()) {
            EvictibleValueEntry<K,V> entry = (EvictibleValueEntry<K,V>) it.next();
            ret += entry.importance;
        }
        return ret;
    }
    ...
    public boolean containsKey(Object key) {
        EvictibleValueEntry<K, V> e = this.delegate.get(key);
    ...
    public boolean containsValue(Object value) {
        ...
        for (EvictibleValueEntry<K, V> e : delegate.values()) {
    ...
    public V get(Object key) {
        EvictibleValueEntry<K, V> e = this.delegate.get(key);
        if ((e == null) || evictIfExpired(e)) return null;
        // modify freq
        EvictibleValueEntry<K, V> ne = new EvictibleValueEntry<K, V>(this,(K)key,e.getValue(),e.weight, e.freq+1, e.evictMs);
        cancelEviction(e);
        scheduleEviction(ne);
        this.delegate.put((K)key, ne);
        return e.getValue();
    }

    /**
     * Add fuzzyGet logic here !!!
     * confidence == 1: exact matching, ==2: fuzzy matching, ==3: no match
     */
    @Override
    public Reply<K,V> fuzzyGet(K key, boolean addcnt) {
        EvictibleValueEntry<K, V> e = this.delegate.get(key);
        Reply<K,V> rep = new Reply<K,V>();
        if (e == null) {
            ConcurrentNavigableMap.Entry<K, EvictibleValueEntry<K, V>> tmpl = this.delegate.floorEntry(key);
            ConcurrentNavigableMap.Entry<K, EvictibleValueEntry<K, V>> tmph = this.delegate.ceilingEntry(key);
            EvictibleValueEntry<K, V> elow = (tmpl != null) ? tmpl.getValue() : null;
            EvictibleValueEntry<K, V> ehigh = (tmph != null) ? tmph.getValue() : null;

            if (elow != null || ehigh != null) {
                rep.confidence = 2;
                rep.higherVal = (ehigh == null) ? null : ehigh.getValue();
                rep.higherKey = (ehigh == null) ? null : ehigh.getKey();
                rep.lowerVal = (elow == null) ? null : elow.getValue();
                rep.lowerKey = (elow == null) ? null : elow.getKey();
                rep.exactVal = null;
                if (addcnt) {
                    if (elow != null) {
                        EvictibleValueEntry<K, V> nelow = new EvictibleValueEntry<K, V>(this, tmpl.getKey(), elow.getValue(), elow.weight, elow.freq + 1, elow.evictMs);
                        cancelEviction(elow);
                        scheduleEviction(nelow);
                        this.delegate.put(tmpl.getKey(), nelow);
                    }
                    if (ehigh != null) {
                        EvictibleValueEntry<K,V> nehigh = new EvictibleValueEntry<K, V>(this,tmph.getKey(),ehigh.getValue(),ehigh.weight, ehigh.freq+1, ehigh.evictMs);
                        cancelEviction(ehigh);
                        scheduleEviction(nehigh);
                        this.delegate.put(tmph.getKey(), nehigh);
                    }
                }
            } else {
                rep.confidence = 3;
            ...
            rep.exactVal = e.getValue();
            rep.confidence = 1;
            rep.higherVal = null;
            rep.lowerVal = null;
            if (addcnt) {
                EvictibleValueEntry<K, V> ne = new EvictibleValueEntry<K, V>(this,key,e.getValue(),e.weight, e.freq+1, e.evictMs);
                cancelEviction(e);
                scheduleEviction(ne);
                this.delegate.put(key, ne);
            }
        }
        return rep;
    }
     ...
     */
    @Override
    public Reply<K, V> traverseGet(K key, boolean addcnt, EntryManipulator manip) {
        EvictibleValueEntry<K, V> e = this.delegate.get(key);
        Reply<K,V> rep = new Reply<K,V>();
        if (e == null) {
            Iterator it = this.delegate.entrySet().iterator();
            double sim = -1, tmp;
            Map.Entry<K, EvictibleValueEntry<K, V>> candidate = null;
            while (it.hasNext()) {
                Map.Entry<K, EvictibleValueEntry<K, V>> entry = (Map.Entry<K, EvictibleValueEntry<K, V>>)it.next();
                tmp = manip.keySimilarity(entry.getKey(), key);
                if (sim < 0 || tmp < sim) {
                    sim = tmp;
                    candidate = entry;
                }
            ...
            if (candidate != null) {
                rep.confidence = 2;
                rep.exactVal = null;
                rep.higherKey = candidate.getKey();
                rep.higherVal = candidate.getValue().getValue();
                if (addcnt) {
                    EvictibleValueEntry<K, V> ne = new EvictibleValueEntry<K, V>(this, candidate.getKey(), candidate.getValue().getValue(),
                            candidate.getValue().weight, candidate.getValue().freq + 1, candidate.getValue().evictMs);
                    cancelEviction(candidate.getValue());
                    scheduleEviction(ne);
                    this.delegate.put(candidate.getKey(), candidate.getValue());
                }
            } else {
                rep.confidence = 3;
                rep.exactVal = null;
     ...
     */
    @Override
    public V put(K key, V value, long weight, long evictMs) {
        EvictibleValueEntry<K, V> e = new EvictibleValueEntry<K, V>(this, key, value, weight, evictMs);
        EvictibleValueEntry<K, V> oe = this.delegate.put(key, e);
        if (oe != null) {
    ...
    public V putIfAbsent(K key, V value) {
        return putIfAbsent(key, value, 1, 0);
    }
     ...
     */
    @Override
    public V putIfAbsent(K key, V value, long weight, long evictMs) {
        while (true) {
            EvictibleValueEntry<K, V> e = new EvictibleValueEntry<K, V>(this, key, value, weight, evictMs);
            EvictibleValueEntry<K, V> oe = this.delegate.putIfAbsent(key, e);
            if (oe == null) {
    ...
    public V remove(Object key) {
        EvictibleValueEntry<K, V> oe = this.delegate.remove(key);
        if (oe != null) {
            // An entry is being removed, cancel its automatic eviction
            cancelEviction(oe);
        }
    ...
    public boolean remove(Object key, Object value) {
        ...
        EvictibleValueEntry<K, V> oe = this.delegate.get(key);
    ...
    public V replace(K key, V value) {
        ...
        EvictibleValueEntry<K, V> e = new EvictibleValueEntry<K, V>(this, key, value, oe.weight, oe.freq, oe.evictMs);
    ...
    public V replace(K key, V value, long evictMs) {
        ...
        // Attempt replacement and schedule eviction if successful
        EvictibleValueEntry<K, V> e = new EvictibleValueEntry<K, V>(this, key, value, oe.weight, oe.freq, evictMs);
        oe = this.delegate.replace(key, e);
        if (oe != null) {
            // An entry is being replaced, cancel the automatic eviction of the
            // old entry
    ...
    public boolean replace(K key, V oldValue, V newValue) {
        ...
        // Avoid replacing an expired entry
        EvictibleValueEntry<K, V> oe = delegate.get(key);
        if ((oe == null) || evictIfExpired(oe) || !oldValue.equals(oe.getValue())) {
            return false;
        }

        // Attempt replacement and schedule eviction if successful
        EvictibleValueEntry<K, V> e = new EvictibleValueEntry<K, V>(this, key, newValue, oe.weight, oe.freq , oe.evictMs);
    ...
    public boolean replace(K key, V oldValue, V newValue, long evictMs) {
        ...
        // Attempt replacement and schedule eviction if successful
        EvictibleValueEntry<K, V> e = new EvictibleValueEntry<K, V>(this, key, newValue, oe.weight, oe.freq , evictMs);
        boolean replaced = delegate.replace(key, oe, e);
        if (replaced) {
            // An entry is being replaced, cancel the automatic eviction of the
            // old entry
    ...
    private boolean evictIfExpired(EvictibleValueEntry<K, V> e) {
     ...
     */
    private boolean evictIfExpired(EvictibleValueEntry<K, V> e, boolean cancelPendingEviction) {
     ...
     * eviction.
     * TODO: in this method, we need to hook with outer cache structure, so as to delete all entries at the same time
     */
    void evict(EvictibleValueEntry<K, V> e, boolean cancelPendingEviction) {
     ...
     * @param
     */
    public void evictMin(){
        Iterator it = this.delegate.entrySet().iterator();
        long min = -1;
        K key = null;
        while (it.hasNext()) {
            Map.Entry<K, EvictibleValueEntry<K, V>> entry = (Map.Entry<K, EvictibleValueEntry<K, V>>)it.next();
            if (min == -1 || entry.getValue().importance < min) {
                min = entry.getValue().importance;
                key = entry.getKey();
            }
        }
        evict(this.delegate.get(key),true);
    }
     ...
     * TODO: reivse scheduler imlementation
     */
    private void scheduleEviction(EvictibleValueEntry<K, V> e) {
     ...
     */
    private void cancelEviction(EvictibleValueEntry<K, V> e) {
    ...
    private final class EntrySet extends AbstractSet<Entry<K, V>> {
        ...
        public boolean remove(Object o) {
            ...
            Entry<K, V> e = (Entry<K, V>) o;
            return ConcurrentMapWithValuedEvictionDecorator.this.remove(e.getKey(), e.getValue());
        }

        @Override
        public int size() {
            return ConcurrentMapWithValuedEvictionDecorator.this.size();
        }
        ...
        @Override
        public void clear() {
            ConcurrentMapWithValuedEvictionDecorator.this.clear();
        }
    }
    ...
    private final class EntryIterator implements Iterator<Entry<K, V>> {
        ...
        private final Iterator<Entry<K, EvictibleValueEntry<K, V>>> iterator = delegate.entrySet().iterator();
        ...
        public synchronized void remove() {
            ...
            ConcurrentMapWithValuedEvictionDecorator.this.remove(currentEntry.getKey(), currentEntry.getValue());
